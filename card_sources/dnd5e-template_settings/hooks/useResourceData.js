...
/**
 * Generic hook for loading any SRD resource from Api.Resources.
 * Handles Blob / string / plain-object responses.
 *
 * @param {object} Api
 * @param {string} resourceKey   - e.g. "5e_items" or "E5_languages"
 * @param {function} extract     - receives the parsed JSON object, returns the array
 *                                 e.g. (d) => d.item ?? d  or  (d) => d.language ?? d
 */
export const useResourceData = (Api, resourceKey, extract) => {
  ...
  const load = React.useCallback(async () => {
    ...
    const p = (async () => {
      ...
      setError(null);
      try {
        const data = await Api.Resources.Read(resourceKey);
